...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      toast.success('Registration successful! Welcome to EcoPoints!');
  ...
  const passwordStrength = (password: string) => {
    ...
    if (score <= 2) return { score, label: 'Weak', color: 'text-error-600' };
    if (score <= 3) return { score, label: 'Fair', color: 'text-warning-600' };
    if (score <= 4) return { score, label: 'Good', color: 'text-eco-600' };
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Join EcoPoints and start earning rewards for sustainable actions
          </p>
        </div>
                      ...
                      {[1, 2, 3, 4, 5].map((level) => (
                        <div
                          ...
                          className={`h-2 w-8 rounded-full ${
                            level <= strength.score ? 'bg-current' : 'bg-gray-200'
                          } ${strength.color}`}
                        />
                      ))}
                    </div>
